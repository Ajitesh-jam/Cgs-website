...
import './components/carousel.css'; // Import carousel CSS
import { gsap } from "gsap/dist/gsap";
...
function Homepage() {
    ...
    const handleSectionClick = (sectionClass) => {
        const activeSection = document.querySelector(`.${sectionClass}`);
        setStackArray(prevStack => [...prevStack, sectionClass]);
    };

    const handleCloseButtonClick = () => {
        setStackArray(prevStack => prevStack.slice(0, -1));
    };

    const openFolder = (e) => {
        const sectionToOpen = stackArray.find(section => section === e.target.parentElement.classList[1]);
        if (!sectionToOpen || !activeSection) {
            setStackArray([...stackArray, e.target.parentElement.classList[1]]);
        } else {
            const indexOfSection = stackArray.findIndex(section => section === activeSection.classList[1]);
            const tempArray = stackArray.slice(indexOfSection + 1);
            setStackArray([...stackArray.slice(0, indexOfSection), ...tempArray]);
        }
    ...
    return (
        <div className="outer-container" id="outer-container">
            {/* Your navigation and sidebar components */}
            <div className="inner-container">
                <div id="Main">
                    <section className="wrapper about-us draggable hidden" id="about-us">
                        <header>About us<button className="close-button">X</button></header>
